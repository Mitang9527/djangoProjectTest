...
# 各环境共用基础配置；环境差异经 global_config (Pydantic 校验) 与 .env 注入。
# 组织顺序：核心→应用→中间件→路由/模板→数据库/缓存/会话→认证/JWT/OIDC→
# API 网关/签名→文件上传/存储→通知/邮件→安全头→国际化→日志→文档→Celery→监控→其他
# =============================================================================
...
# 将 apps & extensions 加入 sys.path（两者均为 import 根）
...
THIRD_PARTY_APPS = [
    ...
    'framework',                           # 内部框架库（事务 Outbox 等 DB 模型所在）
]
...
MIDDLEWARE = [
    ...
    # 7. 多语言（依赖 Session，须在 SessionMiddleware 之后）
    ...
    # 11.5 会话空闲超时（必须在认证之后；已登录用户超阈值无操作则强制登出）
    'framework.security.idle_timeout.IdleTimeoutMiddleware',

    # 12. API 网关（限流、访问日志）——必须放在签名中间件之前，让所有 /api 请求
    # （含被签名拦截的 404/403）都能被网关记录完整访问日志（method/path/status/耗时）。
    ...
    # 13.5 Authorization 头自动补全 Bearer 前缀（手动测试裸 token 免手敲；已带
    #      Bearer/Basic 等方案的头及 X-API-Key 独立头不受影响）
...
# 5. 数据库 / 缓存 / 会话
# 数据库读写分离路由：默认不启用；各环境在定义 DATABASES 后调用 apply_replica()
# 注入副本与路由（薄封装 framework.db.replica.install_replica，避免 dev/prod/test 三处重复 import）。
...
# 多级缓存：Redis 启用且 django-redis 可用时走 Redis，否则回退内存缓存
...
# 会话空闲超时（秒）：已登录用户超过该时长无操作则强制登出；0 表示禁用。
# 与 SESSION_COOKIE_AGE（绝对 7 天硬上限）互补：持续活跃可保活，空闲超阈值即踢。
SESSION_IDLE_TIMEOUT_SECONDS = int(getattr(global_config, "SESSION_IDLE_TIMEOUT_SECONDS", 1800))

# 空闲超时排除路径前缀（不参与计时）；页面超时后的重定向地址。均来自 model.py，可经环境变量覆盖。
...
# ⚠️ 迁移期间 cachalot 会缓存 django_content_type 查询并返回陈旧空结果 → create_contenttypes
# 重复插入 UNIQUE constraint failed。仅 migrate/test 时关闭查询缓存（manage.py test 建测试库
# 也会内部迁移，dev settings 下跑测试同样复现）。不影响运行时缓存。
import sys as _sys
if 'migrate' in _sys.argv or 'test' in _sys.argv:
...
REST_FRAMEWORK = {
    ...
    # 版本控制统一由 URL 路径前缀（/api/v1/、/api/v2/…）实现，由 settings.API_VERSIONS 驱动；
    # 不启用 DRF AcceptHeader 版本化，避免「路径 + header」双机制并存导致路由与文档混乱。
}
...
# 安全护栏：生产环境强制使用独立 JWT_SIGNING_KEY，禁止回退 SECRET_KEY
# （SECRET_KEY 泄露即可伪造任意 JWT；主平台与 AI 服务须配置相同值以跨服务验签）
...
# OIDC 单点登录（SSO）配置；仅在 global_config.oidc.enabled 时启用，未启用不影响启动
...
if OIDC_ENABLED:
    ...
    # OIDC 角色映射：OIDC_ROLE_CLAIM 从 claims 读角色键（默认 'roles'，兼容 IdP 用 'groups'）；
    # OIDC_ROLE_MAP 显式映射 {IdP 角色值: 本地角色 slug}，未配置的 claim 值若与本地角色 slug
    # 同名则直通命中（宽松模式）。仅绑定 tenant 为空的系统角色，未命中忽略（用户保持原角色）。
...
# 7. API 网关 / 签名 / 版本 / 限流
# CORS 跨域：开发环境允许所有来源；生产按 global_config.CORS_ALLOWED_ORIGINS 限制。
...
# API 网关限流配置（env 注入 GATEWAY_THROTTLE_RATE_{IP|USER|TENANT|ANON|ENDPOINT}，
# 对应 model.py global_config；留空回退下列默认值）
...
# 登录限流（IP + 用户名双维度防暴力破解，语义对齐 Fast-Vben-Admin login.py）：
# 窗口内失败达阈值锁定；Redis 故障 fail-open 放行（避免基础设施故障阻断登录）。
LOGIN_RATE_LIMIT_ENABLED = getattr(global_config, "LOGIN_RATE_LIMIT_ENABLED", True)
LOGIN_RATE_LIMIT_MAX_ATTEMPTS = int(getattr(global_config, "LOGIN_RATE_LIMIT_MAX_ATTEMPTS", 0) or 5)
LOGIN_RATE_LIMIT_WINDOW_SECONDS = int(getattr(global_config, "LOGIN_RATE_LIMIT_WINDOW_SECONDS", 0) or 300)
LOGIN_RATE_LIMIT_BLOCK_SECONDS = int(getattr(global_config, "LOGIN_RATE_LIMIT_BLOCK_SECONDS", 0) or 900)

# 注册限流（IP 维度防批量刷注册，键前缀独立于登录；Redis 故障 fail-open）：
# ⚠️ 注册成功【不】清零（防批量开号脚本清零绕过），锁随窗口过期自然解除。
REGISTER_RATE_LIMIT_ENABLED = getattr(global_config, "REGISTER_RATE_LIMIT_ENABLED", True)
REGISTER_RATE_LIMIT_MAX_ATTEMPTS = int(getattr(global_config, "REGISTER_RATE_LIMIT_MAX_ATTEMPTS", 0) or 10)
REGISTER_RATE_LIMIT_WINDOW_SECONDS = int(getattr(global_config, "REGISTER_RATE_LIMIT_WINDOW_SECONDS", 0) or 3600)
REGISTER_RATE_LIMIT_BLOCK_SECONDS = int(getattr(global_config, "REGISTER_RATE_LIMIT_BLOCK_SECONDS", 0) or 3600)

# 密码重置请求限流（IP + email 双维度防邮件轰炸/防枚举；Redis 故障 fail-open）
PASSWORD_RESET_RATE_LIMIT_ENABLED = getattr(global_config, "PASSWORD_RESET_RATE_LIMIT_ENABLED", True)
PASSWORD_RESET_RATE_LIMIT_MAX_ATTEMPTS = int(getattr(global_config, "PASSWORD_RESET_RATE_LIMIT_MAX_ATTEMPTS", 0) or 5)
PASSWORD_RESET_RATE_LIMIT_WINDOW_SECONDS = int(getattr(global_config, "PASSWORD_RESET_RATE_LIMIT_WINDOW_SECONDS", 0) or 3600)
PASSWORD_RESET_RATE_LIMIT_BLOCK_SECONDS = int(getattr(global_config, "PASSWORD_RESET_RATE_LIMIT_BLOCK_SECONDS", 0) or 3600)
# 找回密码邮件中的重置页地址（前端路由），为空则邮件只含 token
PASSWORD_RESET_FRONTEND_URL = getattr(global_config, "PASSWORD_RESET_FRONTEND_URL", "") or ""
# 重置 token 有效期（秒）→ Django PasswordResetTokenGenerator 的 PASSWORD_RESET_TIMEOUT
PASSWORD_RESET_TIMEOUT = int(getattr(global_config, "PASSWORD_RESET_TIMEOUT_SECONDS", 0) or 1800)

# OAuth2 第三方登录（Authorization Code + PKCE）；回调成功后重定向前端地址，为空则回调直接返回 JSON（access/refresh）
OAUTH2_FRONTEND_REDIRECT_URL = getattr(global_config, "OAUTH2_FRONTEND_REDIRECT_URL", "") or ""
# state 有效期（秒）：授权链接生成到回调消费的窗口
OAUTH2_STATE_TTL = int(getattr(global_config, "OAUTH2_STATE_TTL", 0) or 600)
# email 命中既有本地用户自动绑定 / 未匹配自动建号（对齐 OIDC_CREATE_USER）
OAUTH2_AUTO_BIND_EMAIL = getattr(global_config, "OAUTH2_AUTO_BIND_EMAIL", True)
OAUTH2_AUTO_CREATE_USER = getattr(global_config, "OAUTH2_AUTO_CREATE_USER", True)

# MFA TOTP 双因子认证（对齐 Fast-Vben-Admin core/mfa.py）
MFA_TOTP_ISSUER = getattr(global_config, "MFA_TOTP_ISSUER", "") or "Django Enterprise Platform"
MFA_TOTP_VALID_WINDOW = int(getattr(global_config, "MFA_TOTP_VALID_WINDOW", 0) or 1)

# 日志保留 TTL（天数）：审计/登录/操作日志归档清理（manage.py cleanup_logs 消费）
AUDIT_LOG_TTL_DAYS = int(getattr(global_config, "AUDIT_LOG_TTL_DAYS", 0) or 180)
LOGIN_LOG_TTL_DAYS = int(getattr(global_config, "LOGIN_LOG_TTL_DAYS", 0) or 90)
OPERATION_LOG_TTL_DAYS = int(getattr(global_config, "OPERATION_LOG_TTL_DAYS", 0) or 90)
...
# 业务 API 版本列表（URL 路径版本化权威来源）：驱动 /api/<version>/<leaf>/ 自动发现路由
# 及主 urls 是否挂载 /api/v2/users/ 等手动 v2 路由；新增版本在此追加并确保 app 提供 <version>_urls 模块
API_VERSIONS = ['v1']
...
# 排除签名验证的路径：任何公开的 /api 接口都必须在此登记，否则前端调取不到
...
# 生产环境异常处理脱敏：决定 400 字段校验错误(ValidationError)是否脱敏。
# False(默认)→保留字段级 errors（静态校验文案，不含系统内部信息）；True→统一隐藏为「请求参数有误或处理失败」。
# 注意：500 错误在生产始终脱敏（绝不返回异常原文/类名/堆栈/路径，仅写日志），与此开关无关。
...
# 对象存储 (OSS) 配置（framework.storage）：从 global_config.oss 读取。
# backend: local(默认) | aliyun | s3(minio/cos/obs 等兼容)；未启用/SDK 缺失时降级本地磁盘 (MEDIA_ROOT/oss)。
OSS_CONFIG = global_config.oss.model_dump()
STORAGE_BACKEND = os.environ.get("OSS_BACKEND", OSS_CONFIG.get("backend", "local"))

# DEFAULT_FILE_STORAGE：若需 Django 模型 FileField 默认落 OSS 可设
# "framework.storage.django_storage.DjangoOSSStorage"；默认本地文件系统（与 Django 原生一致）。
...
# 文件验证配置：validator 使用 framework.files.upload.validators.FileValidator 内置
# DEFAULT_ALLOWED_TYPES/EXTENSIONS 为权威白名单，本变量为同义冗余，保留以备后续改「settings 驱动白名单」。
...
# 视频类单独放宽上限（容器天然更大，避免被通用 10MB 误伤）；该值须与框架层 validator 的
# DEFAULT_MAX_VIDEO_FILE_SIZE 对齐，且 DATA_UPLOAD_MAX_MEMORY_SIZE 必须 >= 此值 + multipart 开销。
FILE_UPLOAD_MAX_VIDEO_FILE_SIZE = 100 * 1024 * 1024  # 100MB
# 框架层上传 DoS 防护：请求体超限（含所有字段/文件）直接在解析阶段拒绝，防恶意大文件耗尽内存/磁盘。
# 必须 >= 视频上限 + multipart 开销，否则正常大视频被 413；放宽后 DoS 防护由 per-file 上限 + 鉴权兜底。
DATA_UPLOAD_MAX_MEMORY_SIZE = 100 * 1024 * 1024 + 10 * 1024 * 1024  # 110MB（>= 视频上限 + 开销）
# 超过该值流式写入临时文件（降低内存压力）；注意：须 < DATA_UPLOAD_MAX_MEMORY_SIZE，否则大文件解析阶段被 413。
...
# ⚠️ 必须用 `or` 而非 get() 的 default：.env 写 "LOG_LEVEL=" 时环境变量存在但值为空串，
# get() 的 default 不会生效 → 空串会直接传给 loguru。
...
SPECTACULAR_SETTINGS = {
    ...
    # OpenAPI servers：为 Swagger Try it out 与 openapi-ts 客户端提供默认后端地址。
    # paths 已含 /api/v1 前缀，此处只写 host 根；联调/生产用 OPENAPI_INPUT 指向实际环境。
...
# 主动探活任务（core.tasks.probe_dependencies）在依赖组件失败时经 alert_system.AlertEngine
# 发送的渠道；为空仅记录告警历史、不外发。可用渠道如 ["dingtalk", "feishu", "email"]。
...
# DB 连接池配置：可在 settings.DATABASES['default']['_pool']（顶层键，Django 不传给驱动）中覆盖
# （字段：enabled/min_size/max_size/timeout/max_idle/max_lifetime/pre_ping）。
# ⚠️ 千万不要放进 OPTIONS['pool']，否则 OPTIONS 被 **conn_params 展开后会把 pool 传给驱动
# connect()，触发 TypeError。仅 PostgreSQL/MySQL 生效；SQLite 自动走 CONN_MAX_AGE 长连接。

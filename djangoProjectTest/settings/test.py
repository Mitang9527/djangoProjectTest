"""测试环境配置：SQLite 内存库、Redis/Celery 禁用、最快密码哈希、内存邮件后端。"""
import os

# 注意：pytest-django 4.x 在根 conftest.py 导入前就会加载 settings（pytest_load_initial_conftests 钩子），
# 因此测试环境变量必须在 settings 模块自身兜底，不能只依赖 conftest.py 的 setdefault。
...
# 读写分离：默认无 DB_REPLICA_URL，apply_replica 无副作用，保持单库保证 pytest 可跑。
...
# 演示登录默认关闭（测试收敛，不依赖 .env 开关）；需要的测试用 override_settings(ALLOW_DEMO_LOGIN=True)

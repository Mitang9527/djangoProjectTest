...
# ==========================================================================
# 前端 SPA -> Django 后端页面 的单点登录桥接
# --------------------------------------------------------------------------
# Vue 前端用 JWT（存 localStorage），Django 的 /saas/ 后台用 Session Cookie，
# 两套登录态天然不互通。这里用「短时签名票据」做一次中转：
#   1) 前端持 JWT 调 sso/ticket/ 换一张 60 秒有效的票据；
#   2) 浏览器带票据打开 sso/bridge/，后端验签后建立 Session 并 302 到目标页。
# 让票据而非 JWT 出现在 URL 中，避免长效凭证泄漏进浏览器历史与访问日志。
# ==========================================================================

SSO_TICKET_SALT = 'ai_studio.sso.bridge'
SSO_TICKET_MAX_AGE = 60  # 票据有效期（秒），够完成一次跳转即可
SSO_DEFAULT_NEXT = '/saas/'
# 仅允许跳转到这些后端页面前缀，避免本接口沦为开放重定向跳板
SSO_ALLOWED_NEXT_PREFIXES = ('/saas/', '/admin/', '/api/swagger/', '/api/redoc/')


def _safe_next(raw: str) -> str:
    """把 next 参数收敛到白名单内的站内路径。"""
    if not raw:
        return SSO_DEFAULT_NEXT
    # 排除绝对 URL（http://evil.com）与协议相对地址（//evil.com）
    if not raw.startswith('/') or raw.startswith('//'):
        return SSO_DEFAULT_NEXT
    if any(raw.startswith(prefix) for prefix in SSO_ALLOWED_NEXT_PREFIXES):
        return raw
    return SSO_DEFAULT_NEXT


def _sso_error(message: str, status: int = 400) -> HttpResponse:
    """票据校验失败时返回可读 HTML —— 用户是在浏览器窗口里打开的，JSON 不友好。"""
    html = (
        '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8">'
        '<title>跳转失败</title></head>'
        '<body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;'
        'background:#0b0f1a;color:#e8edf7;display:flex;align-items:center;'
        'justify-content:center;height:100vh;margin:0">'
        '<div style="text-align:center;max-width:420px;padding:24px">'
        '<h2 style="margin:0 0 12px;font-size:20px">跳转失败</h2>'
        f'<p style="color:#9aa6c2;margin:0;line-height:1.7">{escape(message)}</p>'
        '</div></body></html>'
    )
...
class SsoTicketView(APIView):
    """签发短时 SSO 票据（需 JWT 认证）。

    票据仅承载 user_id 与签发时间戳，由 SECRET_KEY 签名，无需服务端存储，
    因此在多进程 / 多实例部署下同样可用（不依赖 Redis 或本地内存）。
    """

    permission_classes = [IsAuthenticated]

    def post(self, request):
        signer = TimestampSigner(salt=SSO_TICKET_SALT)
        return Response({
            'ticket': signer.sign(str(request.user.pk)),
            'expires_in': SSO_TICKET_MAX_AGE,
            'bridge_path': '/api/ai_studio/sso/bridge/',
        })


class SsoBridgeView(View):
    """凭票据建立 Django Session，随后重定向到后端页面。

    刻意使用原生 Django View 而非 DRF APIView：本视图返回 302 / HTML，
    不需要 DRF 的认证、渲染器与统一响应格式介入。
    """
    ...
    def get(self, request):
        ...
        if not ticket:
            return _sso_error('缺少票据参数，请从前端「后台管理」入口进入。')

        signer = TimestampSigner(salt=SSO_TICKET_SALT)
        try:
            user_id = signer.unsign(ticket, max_age=SSO_TICKET_MAX_AGE)
        except SignatureExpired:
            return _sso_error('票据已过期（有效期 60 秒），请回到前端重新点击入口。', 403)
        except BadSignature:
            return _sso_error('票据无效或已被篡改。', 403)

        user = User.objects.filter(pk=user_id, is_active=True).first()
        if user is None:
            return _sso_error('用户不存在或已被禁用。', 403)

        # 用户对象并非来自 authenticate()，必须显式指定认证后端
        backends = getattr(settings, 'AUTHENTICATION_BACKENDS', None) or [
            'django.contrib.auth.backends.ModelBackend'
        ]
        django_login(request, user, backend=backends[0])

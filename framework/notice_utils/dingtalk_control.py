...
class DingTalkSendMsg:
    ...
    def xiao_ding(self):
        ...
        try:
            ...
            raise RuntimeError(
                "未安装依赖 dingtalkchatbot，无法发送钉钉消息；"
                "请执行 `pip install dingtalkchatbot` 或将其加入 requirements。"
            ) from e
    ...
    def feed_link(
        ...
        """ FeedLink 二次封装 """
        try:
            from dingtalkchatbot.chatbot import FeedLink
        except ImportError as e:  # pragma: no cover
            raise RuntimeError(
                "未安装依赖 dingtalkchatbot，无法构造 FeedLink；"
                "请执行 `pip install dingtalkchatbot` 或将其加入 requirements。"
            ) from e
        return FeedLink(
            title=title,

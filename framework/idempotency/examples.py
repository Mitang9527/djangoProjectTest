...
def example_1_decorator():
    ...
    from framework.idempotency.core import set_default_backend, LocalMemoryIdempotencyBackend
...
def example_2_fingerprint_conflict():
    ...
    from framework.idempotency import idempotent
    from framework.idempotency.core import set_default_backend, LocalMemoryIdempotencyBackend
    from framework.idempotency.exceptions import IdempotencyConflict
...
def example_4_drf_viewset():
    ...
    # 见 framework.idempotency.drf.IdempotencyKeyMixin（需 Django + DRF 环境）
    ...
    print("[示例4] 见 framework/idempotency/drf.py 完整源码 + 项目 ViewSet 集成示例")

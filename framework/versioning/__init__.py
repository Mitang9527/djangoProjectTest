...
1. **多版本路由**：URL 前缀 / Header / Subdomain 三种方式识别版本
2. **灰度发布**：按 ``Header X-User-Tier`` / ``Cookie`` / ``权重`` 路由
3. **自动降级**：新版未实现某方法时自动 fallback 到旧版
...
**1) settings.py 注册版本**::
    ...
    register_version(VersionSpec(name="v1", status="deprecated", successor="v2", sunset_date="2026-12-31"))
    register_version(VersionSpec(name="v2", status="active", is_default=True))
    register_version(VersionSpec(name="v3-beta", status="canary", canary_weight=10,
                                 canary_match=lambda req: req.headers.get("X-Beta-Tester") == "1"))

    API_VERSIONING = VersioningConfig(default_version="v2", detection_order=("url", "header", "query"), allow_fallback=True)
...
**3) 方法级灰度**::
    ...
    @only_for("v3-beta")
    def experimental_endpoint(request): ...  # 仅 v3-beta 可访问，其余 404
"""
from .core import (
    VersionSpec,
    VersionStatus,

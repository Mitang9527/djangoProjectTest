...
启用方法（任选其一）：
  方式 A（推荐）：在 settings 的 INSTALLED_APPS 中加入本应用
    INSTALLED_APPS = [..., 'utils.db']
    -> utils.db.apps.DBPoolConfig.ready() 自动执行

  方式 B：手动调用（在 wsgi/asgi 启动脚本）
    from utils.db import patch_all_backends
    patch_all_backends()
"""
...
class DBPoolConfig:
    ...
    因为 utils.db 不是 Django app，避免引入额外 INSTALLED_APPS 复杂度。
    ...
    @classmethod
    def ready(cls):
        """Django 自动调用入口（需要在 INSTALLED_APPS 中加入 'utils.db'）"""
...
def enable_db_pool() -> int:
    ...
    用法（任选其一）：
        ...
        from utils.db.apps import enable_db_pool
        enable_db_pool()
    """
...
default_app_config = "utils.db.apps.DBPoolConfig"

...
快速使用::
    ...
    from utils.ConnectServer import SSHClient, TCPClient, ConfigError, TimeoutError
...
迁移说明：
- 旧 `utils/ssh/` 和 `utils/tcp/` 已删除，全部归类到本包
- 旧 `ssh_exceptions.py` / `tcp_exceptions.py` 已合并为 `exceptions.py`
- 调用方需将 `from utils.ssh import X` 改为 `from utils.ConnectServer import X`
- 旧 except SSHError / except TCPError 仍兼容（SSHError/TCPError 仍为合法类型）
- HTTP 客户端请直接用 `utils.http_client`（之前已封装），本包通过转发 re-export 保持兼容
"""
...
# HTTP 客户端（从 utils.http_client 转发，向后兼容旧项目 requestsHTTP.py 风格）
try:
